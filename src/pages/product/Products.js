    ...
    const sortData = () => {
        if (select === "기본순") {
            ...
            const sorted = [...copyList].sort(
                (a, b) => Number(b.goods_price) - Number(a.goods_price)
            );
            setCopyList(sorted);
        } else if (select === "낮은 가격순") {
            const sorted = [...copyList].sort(
                (a, b) => Number(a.goods_price) - Number(b.goods_price)
            );
            setCopyList(sorted);
        } else {
        ...
        <div style={{ paddingBottom: "50px" }}>
            ...
            {productList !== "not product" && copyList !== "not product" && (
                <Pageing
                    count={COUNT}
                    boardPage={boardPage}
                    boardLength={copyList?.length}
                    url={`/product/products/${categoryCode}`}
                />
            )}
        </div>
    );
